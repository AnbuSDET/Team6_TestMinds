...
public class ManageBatchPage extends Constants {
	...
	List<String> sortableHeaders = Arrays.asList("Batch Name", "Batch Description", "Batch Status", "No Of Classes",
			"Program Name");
	...
	public @FindBy(xpath = "//div[@role='dialog']") WebElement dialogBox;
	...
	public boolean validateSortIcons() {
		...
		System.out.println("sortableHeaderSet: " + sortableHeaderSet.size() + ",sortIcons:" + sortIcons.size()
		...
		for (int i = 0; i < sortableHeaderSet.size(); i++) {
			...
			System.out.println("header: " + header + ",headerText:" + headerText
					+ ",sortableHeaderSet.contains(headerText):" + sortableHeaderSet.contains(headerText));
	...
	public boolean batchPopupAllfieldValidation() {
		...
		List<WebElement> popUpElements = Arrays.asList(popUpprogramNamedropDown, popUpBatchname_Prefix,
				popUpBatchname_Sufix, popUpBatchdescription, popUpBatchstatus, popUpBatchnoOfclasses,
				popUpBatchcancelBtn, popUpBatchsaveBtn, popUpBatchcloseBtn);
	...
	public boolean addNewBatch_Mesgvalidation(String ScenarioName) throws InterruptedException, IOException {
		...
		case "withoutMandatoryfields":
			...
			if (prgramerrMesg.contains("required") && batchNameerrMesg.contains("required")
					&& batchStatusErrMesg.contains("required") && noOfClasserrMesg.contains("required")
					&& batchDeErrormesg.contains("required"))
				returnValue = true;
			break;
	...
	public void click_DeleteBatchusingSearch() throws IOException {
		...
		WebElement delbtn = driver
				.findElement(By.xpath("//button[contains(@class, 'p-button-rounded p-button-danger')]"));
